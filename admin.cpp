...
#include <cstdlib> // Added for EXIT_FAILURE
...
const char *mainserverIP = "127.0.0.1"; // IPv4 loopback
...
int login_signup_handle()
{
    ...
    else if (option == 2)
    {
        ...
        int clientSocket = socket(AF_INET, SOCK_STREAM, 0); // IPv4 socket
        if (clientSocket == -1)
        {
            perror("socket");
            return 0;
        ...
        struct sockaddr_in serverAddr{}; // Zero initialize
        serverAddr.sin_family = AF_INET;
        serverAddr.sin_port = htons(mainserverPort); // Use the same port as the server

        if (inet_pton(AF_INET, mainserverIP, &serverAddr.sin_addr) != 1)
        {
...
void act_server()
{
    ...
    serverAddr.sin_addr.s_addr = htonl(INADDR_ANY); // Fixed IPv4 binding
...
int main()
{
    ...
    if (p == 0)
    {
        ...
        struct sockaddr_in serverAddr{}; // Zero initialize
        serverAddr.sin_family = AF_INET;
        serverAddr.sin_port = htons(serverAdmin_client_login); // Choose a port
        serverAddr.sin_addr.s_addr = htonl(INADDR_ANY);        // Fixed IPv4 binding
        ...
        while (user_cnt < limituser)
        {
            ...
            // Process administrator's data (
            std::cout << "Received Client Data:" << std::endl;
            std::cout << "New Client :: "
                         "Username: "
                      << userdata.username << " ==> Signed in " << std::endl;
